...
readBitmapFile :: Display -> Drawable -> String
                  -> IO (Maybe (Dimension, Dimension, Pixmap, Maybe CInt, Maybe CInt))
readBitmapFile display d filename =
  ...
  alloca $ \ y_hot_return -> do
    ecode <- xReadBitmapFile display d c_filename width_return height_return
    ...
    if ecode == 0
      ...
      return (Just (fromIntegral width, fromIntegral height, bitmap, m_x_hot, m_y_hot))
      else return Nothing
